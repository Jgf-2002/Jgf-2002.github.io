...
const experience = [
  {
    ...
    points: [
      "负责 A 股基本面与分析师预期 Alpha 研究，覆盖价值、质量、成长、现金流和营运效率等因子族；梳理并标准化 253 个因子定义，完成 178 个 Dolphin / PySim 可运行实现，并按 IC、RankIC、覆盖率及跨年度稳定性进行评估。",
      "开展 A 股 20 日尺度市场 Beta 研究，检验价格状态、估值、动量、流动性、订单簿和技术形态等候选信号；运用非重叠相位、空检验、联合回归和残差 IC 识别独立增量，完成因子归因与合成。",
      "构建月频宏观状态与多资产配置框架，将货币、信用、增长、通胀及市场确认指标映射至股票、债券、商品和黄金；完成回测、参数敏感性及历史时点审计，交付信号表、状态时间线和研究报告。",
      "建设约 131 GB 基本面与分析师 point-in-time 数据，完成 Dolphin PIT 加载、Tushare / 聚源 JY 字段统一、PySim 批量仿真和每日质量监控，为研究提供可复现的数据基础。",
    ],
  },
  {
    ...
    <main>
      ...
      <div className="page-shell">
        <aside className="profile-sidebar" aria-label="个人信息">
          ...
          <section className="sidebar-section sidebar-facts">
            <h2>学术概况</h2>
            <p><strong>4.16 / 5.0</strong><span>本科 GPA · 专业前 5%</span></p>
            <p><strong>2 篇</strong><span>已发表论文</span></p>
            <p><strong>全国二等奖</strong><span>大学生电子设计竞赛</span></p>
          </section>
        ...
        <div className="resume-content">
          <section className="resume-section intro-section">
            ...
            <p>本科毕业于东北大学自动化专业（郎世俊实验班）。本科阶段参与医学图像识别、滑模控制、电子系统测量和智能调度研究；目前同时开展 A 股 Alpha 因子、市场 Beta、宏观配置及 point-in-time 数据工程工作。</p>
            <dl className="research-lines">
              <div><dt>当前研究</dt><dd>博弈论 · 马尔可夫随机过程 · 滤波与控制</dd></div>
              <div><dt>方法基础</dt><dd>概率建模 · 机器学习 · 智能优化 · 实验验证</dd></div>
              <div><dt>应用方向</dt><dd>量化研究 · 数据工程 · 系统验证</dd></div>
            </dl>
          </section>
          ...
          <section className="resume-section">
            ...
            <div className="timeline-list">
              {experience.map((item, index) => index === 0 ? (
                <details className="timeline-entry expandable-experience" key={item.org}>
                  <summary>
                    <time>{item.date}<small>可展开查看</small></time>
                    <div>
                      <div className="experience-title"><h3>{item.org}</h3><span className="expand-cue">研究内容 <b>＋</b></span></div>
                  ...
                  <div className="experience-expanded">
                    ...
                    <figure className="experience-figure">
                      ...
                      <figcaption><span>实习研究内容示意图</span><a href="/quant-research-architecture.svg" target="_blank" rel="noreferrer">查看大图 <External /></a></figcaption>
                    </figure>
                  </div>
                </details>
              ) : (
          ...
          <section className="resume-section skills-section">
            ...
            <div className="personal-note"><strong>个人量化实践</strong><p>2025 年 5 月至今，A 股个人模拟盘累计收益 47.97%，最大回撤 6%。该记录仅用于研究展示，不构成投资建议或收益承诺。</p></div>
          </section>
        </div>
      </div>
